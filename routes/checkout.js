...
router.route('/delivery/:id')
    .post(function (req, res, next) {
        ...
        RunQuery(selectQuery, function (rows) {
            ...
            req.session.deliveryType = req.body.inlineRadioOptions;
            req.session.paymentType = req.body.inlineRadioOptions2;
    ...
    .get(function (req, res, next) {
        ...
        for (var item in req.session.cart) {
            if (req.session.cart[item].quantity > 0) {
                subTotal += req.session.cart[item].productTotal
            }
        }
        var insertQuery = '\
            INSERT INTO Orders\
            ...
            "'" + req.session.deliveryType + "'" + ", " +
            "'" + req.session.paymentType + "'" + ", " +
            "'" + req.session.Observation + "'" + ", " +
            subTotal + ', ' +
            req.session.cartSummary.discount + ', ' +
            req.session.cartSummary.shipCost + ', ' +
            subTotal + ', NOW(), \'Pedido Realizado\');';
