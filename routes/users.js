...
module.exports = function (app, passport) {
    ...
    app.get('/sign-in-checkout', function (req, res) {
        // render the page and pass in any flash data if it exists
        if (req.session.inCheckOut){
            var checkOutNoti = '';
            req.session.inCheckOut = false;
        ...
        res.render('sign-in-checkout', contextDict);
    });

    app.post('/sign-in-checkout', passport.authenticate('sign-in', {
        successRedirect: '/cart', // redirect to the secure profile section
        failureRedirect: '/sign-in-checkout', // redirect back to the signup page if there is an error
        failureFlash: true // allow flash messages
    }));
    ...
    app.get('/sign-up-checkout', function (req, res) {
        // render the page and pass in any flash data if it exists
        var checkOutNoti = '';
        var contextDict = {
            title: 'Cadastro',
        ...
        res.render('sign-up-checkout', contextDict);
    });

    // process the signup form
    app.post('/sign-up-checkout', passport.authenticate('sign-up', {
        successRedirect: '/cart', // redirect to the secure profile section
        failureRedirect: '/sign-up-checkout', // redirect back to the signup page if there is an error
        failureFlash: true // allow flash messages
    }));
